...
public class ArraySampleAdvAsmProxy extends ArraySample implements AdvRuntimeReferNameObject {
	...
	@Override
	public SimplePojoClass[] getSimplePojoClassSampleArray() {

		AdvContext context = _contextThreadLocal.get();
		ConsumerWithException<MethodCode> objEval = context.resolve(this);
		byte codeIndex = context.push(SimplePojoClass[].class, c -> {
			objEval.accept(c);
			c.VIRTUAL(ArraySample.class, "getSimplePojoClassSampleArray").return_(SimplePojoClass[].class).INVOKE();
		});
		byte magicNumber = (byte) (MAGIC_CODES_NUMBER + codeIndex);
		SimplePojoClass simplePojoClassSample = null;
		SimplePojoClass[] tarray = new SimplePojoClass[1];

		if (Adv.canProxy(SimplePojoClass.class)) {
			simplePojoClassSample = Adv.buildProxyClass(SimplePojoClass.class, magicNumber);
	...
	@Override
	public void setSimplePojoClassSampleArray(SimplePojoClass[] param0) {
		AdvContext context = _contextThreadLocal.get();
		ConsumerWithException<MethodCode> eval_param0 = context.resolve(param0);
		ConsumerWithException<MethodCode> objEval = context.resolve(this);
		context.execLine(c -> {
			...
			c.VIRTUAL(ArraySample.class, "setSimplePojoClassSampleArray").parameter(SimplePojoClass[].class).INVOKE();
		});
	}
