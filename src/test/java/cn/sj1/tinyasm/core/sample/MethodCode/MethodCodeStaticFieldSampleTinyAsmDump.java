...
public class MethodCodeStaticFieldSampleTinyAsmDump {
	...
	protected void _getAll(ClassBody classBody) {
		...
		code.LOADConst(0);
		code.ARRAYLOAD();
		Label label46OfIFEQ = new Label();
		code.IFEQ(label46OfIFEQ);
		code.LOADConst(0);
		Label label47OfGOTO = new Label();
		code.GOTO(label47OfGOTO);

		code.visitLabel(label46OfIFEQ);
		code.LOADConst(1);

		code.visitLabel(label47OfGOTO);
		code.ARRAYSTORE();
