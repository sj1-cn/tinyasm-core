...
public class AdvMagic {
	...
	public static <T> T build(String name, Class<T> builderClass) {
		AdvClassBuilder classBuilder = Adv.public_class_(name).extends_(Clazz.of(builderClass.getGenericSuperclass()))
				.implements_(Adv.of(c -> Clazz.of(c), builderClass.getGenericInterfaces())).enterClassBody();
